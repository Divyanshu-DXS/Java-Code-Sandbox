package The4Pillars.Abstraction.Interface.SocialMediaExample.Classes;

import The4Pillars.Abstraction.Interface.SocialMediaExample.Interfaces.EntertainmentBasedSocialMedia;
import The4Pillars.Abstraction.Interface.SocialMediaExample.Interfaces.HybridSocialMedia;
import The4Pillars.Abstraction.Interface.SocialMediaExample.Interfaces.SocialMedia;

import java.util.function.DoubleToIntFunction;

public class Facebook extends HybridSocialMedia implements SocialMedia, EntertainmentBasedSocialMedia {

    int numOfVideos=0;
    int numOfPhotos=0;
    int numOfPosts=numOfPhotos+numOfVideos;
    int numOfConnections=0;
    String profileName;

    public Facebook(String profileName) {
        this.profileName = profileName;
    }

    public Facebook() {
    }

    @Override
    public void memes() {
        System.out.println("Posted a meme!!");
        numOfPhotos++;
    }

    @Override
    public void reels() {
        System.out.println("Posted a reel");
        numOfVideos++;

    }

    @Override
    public void checkVideoLength() {

        System.out.println("Suggest Video Length for the platform");
    }

    @Override
    public void postPictures() {
        System.out.println("Posted a picture");
        numOfPhotos++;
    }

    @Override
    public void pinPhotos() {
        System.out.println("Photos Pinned!");
    }

    @Override
    public void addToMoodBoard() {
        System.out.println("Added to your moodboard");
    }

    @Override
    public void sendConnectionRequest() {
        System.out.println("Connection Request Sent!");
        numOfConnections++;

    }

    @Override
    public void approveConnectionRequestReceived() {

        System.out.println("Approve Connecton request -- Y/N");
        // Logic here
        numOfConnections++;
    }

    @Override
    public int getNumOfConnections() {
        return numOfConnections;
    }

    @Override
    public void uploadProfilePhoto() {
        System.out.println("ProfileP Photo Uploaded");
    }

    @Override
    public void makePost() {

        System.out.println("posted");
        // if video numOfVideo++
        // if photo numOfPhoto++
    }

    @Override
    public void openMessenger() {
        System.out.println("messenger Launched !");
    }

    @Override
    public String getProfileName() {
        return profileName;
    }

    @Override
    public void makeShorts() {
        System.out.println("Make Short Videos 1-2 min");
        numOfVideos++;
    }

    @Override
    public void makeReels() {
        System.out.println("Make Reel Videos 1 min");
        numOfVideos++;

    }

    @Override
    public void makeMovie() {
        System.out.println("Make Movie Videos > 2 min");
        numOfVideos++;

    }

    @Override
    public void cutClips() {
        System.out.println("Cut clips from videos");

    }
    public int getNumOfPhotos(){
        return numOfPhotos;
    }
    public int getNumOfVideos(){
        return numOfVideos;
    }
}
